...
export default function ProductInput({ addEntry }: ProductInputProps) {
  ...
  const controlledSetProductInfo = (productInfo: { name: string; code: string; photoUrl: string; price: number; }) => {
    ...
    <div className="shadow-xl p-4 rounded-md bg-white">
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex flex-col gap-2 flex-1">
          <h1 className="text-2xl font-bold">Ingresar producto</h1>
          ...
          <div className="flex flex-row gap-1 justify-start items-center">
            <button className=" bg-red-500 text-white rounded-md w-8 h-8" 
            onClick={() => setProductInfo({
              ...productInfo,
              quantity: productInfo.quantity - 1 < 0 ? 0 : productInfo.quantity - 1,
            })
            }>
              -
            </button>
            ...
            />
            <button className=" bg-blue-500 text-white rounded-md w-8 h-8" 
            onClick={() => setProductInfo({
              ...productInfo,
          ...
          <div className="prices">
            ...
            <p className="text-lg font-bold">Total: ${productInfo.price * productInfo.quantity}</p>
          </div>
          <button className="bg-green-500 text-white rounded-md p-2 w-full" onClick={() => controlledAddEntry(productInfo)}>
            Agregar
          </button>
        </div>
        <div className="rounded-md grid place-items-center ">
          <img src={productInfo.photoUrl} className="rounded-md md:w-full object-cover  w-60 h-60 max-w-full" alt="" />
        </div>
      </div>
      <DialogSearch
        table="producto"
